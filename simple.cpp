...
void _do(int * data, int* nelems, int * indices, int * myindices, int nelem){
...
int main(int argc, char ** argv){
  ...
  */
  ...
  for(int iepoch=0;iepoch<10;iepoch++){
    ...
    Timer t;
    for(int i=0;i<NTHREAD;i++){
      numa_run_on_node(i % 2);
      // The following two strategies are one for Cyclades and one for Hogwild!
      threads.push_back(std::thread(
				    _do, &indices[0], &p_nelems[0], &p_examples[0], numa_aware_indices[i], NELEMS[i]
				    //_do, &indices[0], &p_nelems[0], &p_examples[0], numa_aware_indices_hogwild[i], NELEMS_HOGWILD[i]
      ));
    }
    for(int i=0;i<NTHREAD;i++){
      threads[i].join();
  ...
  for(int i=0;i<NTHREAD;i++){
    std::cout << NELEMS[i] << std::endl;
  }
  //std::cout << "~~~~~~~" << std::endl;
  for(int i=0;i<NTHREAD;i++){
    std::cout << NELEMS_HOGWILD[i] << std::endl;
  }
  return 0;
}
